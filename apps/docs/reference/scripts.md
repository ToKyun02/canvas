...
docs 배포는 **GitHub Actions에서만** 자동 실행됩니다. `main` 브랜치에 `apps/docs/**` 변경이 push되면 `.github/workflows/deploy-docs.yml`이 빌드 후 Firebase Hosting에 배포합니다.

Firebase 설정: `apps/docs/firebase.json`, `apps/docs/.firebaserc`
...
`apps/docs/.firebaserc`의 프로젝트 ID를 실제 Firebase 프로젝트로 설정하세요.
